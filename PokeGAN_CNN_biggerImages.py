...
class PokeGAN_CNN():
    ...
    def build_generator(self):
        ...
        model.add(Conv2DTranspose(60,(3,3),strides=2,padding='same'))
        model.add(BatchNormalization(momentum=0.8))
        model.add(UpSampling2D())
        model.add(Conv2DTranspose(120,(3,3),strides=2,padding='same'))
        model.add(BatchNormalization(momentum=0.8))
    ...
    def train(self, epochs, batch_size=128, sample_interval = 50):
        ...
        X_train = np.load('allOriginal150.npy')
        ...
        #Create noise images to keep consistent tests of the generator
        sample_noise = np.random.normal(0, 1, (25,self.latent_dim))
        ...
        for epoch in range(epochs):
            ...
            if epoch % sample_interval == 0:
                self.sample_images(epoch, summary_stats, sample_noise)
    
    def sample_images(self, epoch, summary_stats, noise):
        r, c = 5, 5
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("biggerImages/%d.png" % epoch)
        plt.close()
        ...
        fig1.savefig('biggerPlots/%d.png' % epoch,dpi=100)
        plt.close()
        
        
if __name__== '__main__':
    gan = PokeGAN_CNN()
    gan.train(epochs=1001,batch_size=100, sample_interval=50)