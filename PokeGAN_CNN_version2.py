# -*- coding: utf-8 -*-
"""
Created on Mon Sep 14 14:00:53 2020
...
class PokeGAN_CNN():
    def __init__(self):
        ...
        loss = BinaryCrossentropy(label_smoothing=.05)
        self.discriminator.compile(loss = loss,
                                   optimizer = optimizer, metrics = ['accuracy'])
        self.generator = self.build_generator()
        
        optimizer = Adam(0.00008, 0.5)
    ...
    def build_generator(self):
        ...
        model.add(LeakyReLU(alpha=0.2))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Reshape((9,15,1)))
        model.add(Conv2DTranspose(120,(5,5),strides=1,padding='valid'))
        model.add(BatchNormalization(momentum=0.8))
    ...
    def train(self, epochs, batch_size=128, sample_interval = 50):
        ...
        for epoch in range(epochs):
            ...
            # imgs, = datagen.flow(imgs,batch_size=batch_size)
            # gen_imgs, = datagen.flow(gen_imgs,batch_size=batch_size)
            
            # d_loss_real = self.discriminator.train_on_batch(imgs, valid)
            # d_loss_fake = self.discriminator.train_on_batch(gen_imgs, fake)
            # d_loss = 0.5 * np.add(d_loss_real, d_loss_fake)
            
            for imgs1 in datagen.flow(imgs,batch_size=batch_size):
                d_loss_real = self.discriminator.train_on_batch(imgs1, valid)
                break
                
            for gen_imgs1 in datagen.flow(gen_imgs,batch_size=batch_size):
                d_loss_fake = self.discriminator.train_on_batch(gen_imgs1, fake)
                break
...
if __name__== '__main__':
    gan = PokeGAN_CNN()
    gan.train(epochs=5001,batch_size=200, sample_interval=100)